...
class LoginScreen(QWidget):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QLabel#uidLabel {
                ...
                border: 1px solid #003a55; border-left: 3px solid #00bfff;
                border-radius: 3px;
            }
            QLabel#errorLabel {
                color: #f87171; font-family: 'Courier New'; font-size: 11px;
                letter-spacing: 2px; padding: 6px 10px; background: #1a0a0a;
                border: 1px solid #3a1010; border-left: 3px solid #f87171;
                border-radius: 3px;
            }
    ...
    def attempt_login(self):
        ...
        if username in VALID_USERS and VALID_USERS[username]["password"] == password:
            uid = VALID_USERS[username]["uid"]
            self.error_label.hide()
            self.uid_label.setText(f"🔑  Account UID: {uid}")
            self.uid_label.show()
            self.login_btn.setEnabled(False)
            QTimer.singleShot(1000, lambda: self.login_success.emit(username, uid))
        else:
            self.uid_label.hide()
...
class SharePanel(QFrame):
    ...
    def set_current_url(self, url: str):
        self.current_url_field.setText(url)
...
class SmoothWebEngineView(QWebEngineView):
    ...
    def _setup_smooth_rendering(self):
        ...
        # ── FIX 1: Allow mixed/insecure content so CDN & redirect chains don't get blocked ──
...
class Browser(QMainWindow):
    def __init__(self, username: str = "", uid: str = ""):
        ...
        # ── FIX 2: Spoof a real Chrome user-agent so sites don't block the embedded engine ──
        ...
        self.setStyleSheet("""
            ...
            QTabBar::close-button { margin-right: 500px; }
            QComboBox {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
    ...
    def toggle_intercept(self, checked):
        ...
        self.intercept_action.setText(f"🛡️ Intercept: {'ON' if checked else 'OFF'}")

    def intercept_url(self, navigation_type, url, is_main_frame):
        if url.toString().startswith(self.home_identifier):
            return True
        if self.intercept_enabled and is_main_frame:
            reply = QMessageBox.question(
                self, 'Armor Shield Intercept',
                f"Safety Check: Allow navigation to?\n\n{url.toString()}",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
    ...
    def home_html(self):
        ...
        <head>
            <style>
                body { margin: 0; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; font-family: 'Segoe UI', sans-serif; background: radial-gradient(circle at center, #1a0033 0%, #05050a 100%); color: #ffffff; overflow: hidden; position: relative; }
                .shield { position: absolute; width: 550px; height: 550px; border-radius: 50%; display: flex; justify-content: center; align-items: center; z-index: 0; }
                .shield::before { content: ""; position: absolute; width: 100%; height: 100%; border-radius: 50%; border: 4px dashed rgba(139, 92, 246, 0.3); animation: rotateCounter 20s linear infinite; }
                .shield::after { content: ""; position: absolute; width: 85%; height: 85%; border-radius: 50%; border: 2px solid rgba(59, 130, 246, 0.1); border-top: 4px solid rgba(168, 85, 247, 0.4); border-bottom: 4px solid rgba(59, 130, 246, 0.4); animation: rotateClockwise 10s linear infinite; }
                ...
                .glow-text { font-size: 50px; font-weight: 800; margin-bottom: 30px; background: linear-gradient(90deg, #a855f7, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; filter: drop-shadow(0 0 10px rgba(168, 85, 247, 0.4)); animation: pulse 3s infinite ease-in-out; position: relative; z-index: 1; }
                .search-container { display: flex; background: rgba(255,255,255,0.05); padding: 15px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(20px); box-shadow: 0 10px 30px rgba(0,0,0,0.5); position: relative; z-index: 1; }
                input { width: 450px; padding: 16px; font-size: 18px; border: none; border-radius: 12px; outline: none; background: #0f172a; color: white; transition: 0.3s; border: 1px solid #334155; }
                input:focus { border-color: #8b5cf6; box-shadow: 0 0 15px rgba(139, 92, 246, 0.3); }
                button { padding: 14px 25px; font-size: 16px; margin-left: 12px; border-radius: 12px; border: none; cursor: pointer; background: linear-gradient(90deg, #8b5cf6, #3b82f6); color: white; font-weight: bold; transition: 0.3s; }
            </style>
        </head>
        <body>
            ...
            <div class="search-container">
                <input type="text" id="searchBox" placeholder="Type a URL or search securely..." autofocus>
